...
// 引力小怪（Hover-Over-Head 版）：在玩家头顶附近悬停，释放 3 秒范围引力吸附周围刚体（含玩家/道具）。
...
namespace FadedDreams.Enemies
{
    ...
    public class EnemyGraviton2D : MonoBehaviour, IDamageable
    {
        ...
        public float pullForcePerSecond = 40f;    // 每秒向心力
        public float attractDuration = 3.0f;
        public float skillCooldown = 5.0f;
        public LayerMask affectMask = ~0;         // 吸引刚体的层（含玩家）
        public UnityEvent onGravityStart;
        public UnityEvent onGravityStop;
        public GameObject vfxAttractLoop;         // 循环特效（可选）
        public GameObject vfxAttractBurst;        // 启动爆发特效（可选）
        public Light2D pulseLight;                // 脉冲光（URP 可选）
