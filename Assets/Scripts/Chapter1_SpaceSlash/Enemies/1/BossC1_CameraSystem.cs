// BossC1_CameraSystem.cs
// 相机系统 - 负责BOSS战相机应用、玩家为中心的构图和边界检测
...
{
    /// <summary>
    /// BossC1相机系统 - 负责BOSS战相机应用、玩家为中心的构图和边界检测
    /// </summary>
    [DisallowMultipleComponent]
    public class BossC1_CameraSystem : MonoBehaviour
    {
        ...
        public bool enableBoundaryDetection = true;
        public float boundaryMargin = 2f;
        public LayerMask boundaryMask = -1;
        ...
        // 事件
        public event System.Action OnBossCameraActivated;
        public event System.Action OnBossCameraDeactivated;
        public event System.Action OnCameraTransitionStarted;
        public event System.Action OnCameraTransitionCompleted;
        ...
        public void ActivateBossCamera()
        {
            ...
            if (verboseLogs)
                Debug.Log("[BossC1_CameraSystem] Activating boss camera");
        ...
        public void DeactivateBossCamera()
        {
            ...
            if (verboseLogs)
                Debug.Log("[BossC1_CameraSystem] Deactivating boss camera");
        ...
        private IEnumerator CameraTransitionCoroutine(bool toBossCamera)
        {
            ...
            if (verboseLogs)
                Debug.Log($"[BossC1_CameraSystem] Camera transition completed: {(toBossCamera ? "Boss" : "Normal")}");
        }
        ...
        private void UpdateCameraPosition()
        {
            ...
            {
                targetPosition = ValidateCameraPosition(targetPosition);
            }
        ...
        /// 验证相机位置
        /// </summary>
        private Vector3 ValidateCameraPosition(Vector3 targetPosition)
        {
            // 检查边界碰撞
        ...
        {
            if (!enableBoundaryDetection) return true;

            // 检查边界碰撞
            Collider2D hit = Physics2D.OverlapCircle(position, boundaryMargin, boundaryMask);
            return hit == null;
        }
        ...
        public void SetAnchorPoint(Transform anchor)
        {
            ...
            if (verboseLogs)
                Debug.Log($"[BossC1_CameraSystem] Anchor point set: {(anchor != null ? anchor.name : "null")}");
        }
        ...
        public void ClearAnchorPoint()
        {
            ...
            if (verboseLogs)
                Debug.Log("[BossC1_CameraSystem] Anchor point cleared");
        }
        ...
        public void RestoreOriginalCamera()
        {
            ...
            if (verboseLogs)
                Debug.Log("[BossC1_CameraSystem] Original camera settings restored");
        }
        ...
        public void ResetCameraSystem()
        {
            ...
            if (verboseLogs)
                Debug.Log("[BossC1_CameraSystem] Camera system reset");
        }
